...
FROM review, restaurant
WHERE review.restaurant_id = restaurant.id GROUP BY name;
...
from restaurant, review, reviewer WHERE review.restaurant_id = restaurant.id AND review.reviewer_id = reviewer.id;


--Get the average stars given by each reviewer. (reviewer name, average star rating)
SELECT reviewer.name, avg(review.stars) from reviewer, review WHERE review.reviewer_id = reviewer.id GROUP BY reviewer.name;


-- Get the lowest star rating given by each reviewer. (reviewer name, lowest star rating)
SELECT reviewer.name, min(review.stars) from reviewer, review  WHERE review.reviewer_id = reviewer.id GROUP BY reviewer.name;


-- Get the number of restaurants in each category. (category name, restaurant count)
SELECT restaurant.category, count(restaurant) from restaurant GROUP BY restaurant.category;


-- Get number of 5 star reviews given by restaurant. (restaurant name, 5-star count)
SELECT restaurant.name, count(stars) from restaurant, review WHERE review.restaurant_id = restaurant.id AND review.stars = 5 GROUP BY restaurant.name;


-- Get the average star rating for a food category. (category name, average star rating)
SELECT restaurant.category, avg(stars) from restaurant, review WHERE review.restaurant_id = restaurant.id GROUP BY restaurant.category;
